...
const runImageDiffAfterScreenshot = async (screenshotConfig) => {
    ...
    const { currentTestTitle, screenshotsFolder, isUpdateSnapshots, customSnapshotsDir, specFileRelativeToRoot, customDiffDir, e2eSpecDir, } = options;
    ...
    const snapshotDotPath = node_path_1.default.join(snapshotsDir, `${snapshotName}${SNAP_EXT}`);
    ...
    const diffDotPath = node_path_1.default.join(diffDir, `${snapshotName}${DIFF_EXT}`);
